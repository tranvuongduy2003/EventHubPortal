...
export function useRolePermissionsTable({
  onChangePermission,
  functions,
}: RolePermissionsTableHookProps) {
    ...
    const column: ColumnType<IRolePermission | AnyObject> = {
      ...
      render: (ids: string[], permission) => (
        <Checkbox
          ...
          onChange={(event) =>
            onChangePermission(permission.roleId, f.id, event.target.checked)
          }
        />
      ),
    };
  ...
  const columns: ColumnsType<IRolePermission | AnyObject> = [
    {
      ...
      colSpan: 1,
      render: (value: string) => (
        <span className="inline-block h6 !text-sm max-w-[155px]">{value}</span>
      ),
    },
    ...functionColumns,
  ];
